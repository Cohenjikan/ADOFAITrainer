#!/bin/bash
# ADOFAI Trainer (冰与火之舞修改器) · macOS + Linux 一键安装（原生，无需 BepInEx / Rosetta）
#
# 懒人一键（在「终端」粘贴运行）：
...
#
# 与「节奏医生修改器 Mac 版」同法：平台无关的托管代码（修改器逻辑 + Mono.Cecil 织入器）在
# macOS 与 Linux 上完全一致；本脚本只把「平台相关」的东西（游戏目录布局、Steam 位置、进程名、
# 日志路径、缺包提示）抽成变量，主逻辑两个平台共用。
set -euo pipefail

REPO_GIT="https://github.com/Cohenjikan/ADOFAITrainer.git"
export DOTNET_ROOT="${DOTNET_ROOT:-$HOME/.dotnet}"
export PATH="$DOTNET_ROOT:$PATH"

# Steam 安装目录下的游戏文件夹名（macOS / Windows / Linux 一致，来自 appmanifest 的 installdir）
GAME_DIR_NAME="A Dance of Fire and Ice"

# --- 平台检测：把所有平台相关的取值集中在这里，下面主逻辑只用这些变量 ---
#   GAME_SUBPATH  : "$GAME_DIR_NAME"/ 下到 Managed/ 的相对子路径（macOS 是 .app 包，Linux 是 *_Data）
...
case "$OS" in
  Darwin)
    GAME_SUBPATH="ADanceOfFireAndIce.app/Contents/Resources/Data/Managed"
    GAME_PROC="ADanceOfFireAndIce.app/Contents/MacOS/ADanceOfFireAndIce"
    PLAYER_LOG="$HOME/Library/Logs/7th Beat Games/A Dance of Fire and Ice/Player.log"
    ...
    ;;
  Linux)
    # NOTE: Linux 分支为「顺路」实现，作者无 Linux 真机验证。ADOFAI 的原生 Linux 版可执行名未经确认，
    #       这里按 macOS 二进制名推测为 ADanceOfFireAndIce.x86_64；若你的版本不同，进程检测可能失准
    #       （只影响「游戏运行中」的拦截，不影响织入本身）。详见 README 的 Linux 说明。
    GAME_SUBPATH="A Dance of Fire and Ice_Data/Managed"
    GAME_PROC="ADanceOfFireAndIce.x86_64"
    PLAYER_LOG="$HOME/.config/unity3d/7th Beat Games/A Dance of Fire and Ice/Player.log"
    STEAM_ROOTS=(
...
if [ -z "$HERE" ] || [ ! -f "$HERE/mac/ADOFAITrainerMac/ADOFAITrainerMac.csproj" ]; then
  if ! command -v git >/dev/null 2>&1; then
    echo "ERROR: 需要 git。$GIT_HINT"; exit 1
  fi
  SRC="${ADOFAIT_SRC:-$HOME/.adofaitrainer-mac/src}"
  echo "==> 获取源码 -> $SRC"
...
for root in "${STEAM_ROOTS[@]}"; do
  CANDS+=("$root/steamapps/common/$GAME_DIR_NAME/$GAME_SUBPATH")
  vdf="$root/steamapps/libraryfolders.vdf"
  if [ -f "$vdf" ]; then
    # vdf 里每个库一行：  "path"\t\t"/some/Steam Library" —— 按引号切分取第 4 段即路径。
    # 只读解析，awk 在 macOS(BSD) 与 Linux(GNU/mawk) 行为一致；|| true 防止 set -e 误杀。
    while IFS= read -r libpath; do
      [ -n "$libpath" ] && CANDS+=("$libpath/steamapps/common/$GAME_DIR_NAME/$GAME_SUBPATH")
...
# Linux: derive the real executable name from the resolved game dir (Unity ships <name>.x86_64 /
# <name>.x86 next to <name>_Data) instead of trusting a hardcoded guess — makes the running-check
# robust to whatever ADOFAI's native build is actually called. Falls back to GAME_PROC if none found.
if [ "$OS" = "Linux" ]; then
  GAME_ROOT="$(cd "$MANAGED/../.." 2>/dev/null && pwd || true)"
  if [ -n "${GAME_ROOT:-}" ]; then
    for exe in "$GAME_ROOT"/*.x86_64 "$GAME_ROOT"/*.x86; do
      [ -f "$exe" ] && { GAME_PROC="$(basename "$exe")"; break; }
    done
  fi
fi
...
echo "==> 编译修改器 DLL"
dotnet build -c Release "$HERE/mac/ADOFAITrainerMac/ADOFAITrainerMac.csproj" -p:Managed="$MANAGED" | tail -3
...
echo "==> 织入 Assembly-CSharp.dll（把 Loader.Init 织进 ADOStartup.Startup）"
dotnet run -c Release --project "$HERE/mac/Patcher/Patcher.csproj" -- \
  "$MANAGED" \
  "$HERE/mac/ADOFAITrainerMac/bin/Release/ADOFAITrainerMac.dll" \
  "$HARMONY"

cat <<EOF

==> 完成！正常通过 Steam 启动游戏，进任意关卡按 F3 开/关菜单。
    验证：  grep ADOFAITrainerMac "$PLAYER_LOG"
            期望：  ... v1.41 loaded · ... · Menu key = F3 · patches ok=1 fail=0
    卸载：  curl -fsSL https://raw.githubusercontent.com/Cohenjikan/ADOFAITrainer/refs/heads/main/uninstall.sh | bash
