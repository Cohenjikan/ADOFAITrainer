# ADOFAI Trainer (冰与火之舞修改器) · Windows 一键卸载 (PowerShell)
#
...
Write-Host ""
Write-Host "==== 冰与火之舞修改器  Windows 卸载 ====" -ForegroundColor Cyan
Write-Host ""
...
$game = Find-Game
if (-not $game) { $game = (Read-Host '没自动找到游戏，请粘贴《A Dance of Fire and Ice》目录后回车').Trim().Trim('"') }
if (-not (Test-Path (Join-Path $game 'A Dance of Fire and Ice.exe'))) { Write-Host "[错误] 目录无效。" -ForegroundColor Red; return }

$dll = Join-Path $game 'BepInEx\plugins\ADOFAITrainer.dll'
