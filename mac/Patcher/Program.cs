...
// Weaves a single `ADOFAITrainerMac.Loader.Init()` call into the game's ADOStartup.Startup so the
// trainer loads itself at boot — no BepInEx / Doorstop needed. ADOStartup.Startup is ADOFAI's
// boot routine (locale / Steam SDK / platform / save / resolution), the analog of Rhythm Doctor's
// RDStartup.Setup. Idempotent: always re-derives from a pristine backup, so running it repeatedly
// never double-injects.
//
// Usage: Patcher <ManagedDir> <ADOFAITrainerMac.dll> <0Harmony.dll>
class Program
{
    const string InjectType = "ADOStartup";
    const string InjectMethod = "Startup";

    static int Main(string[] args)
    {
        if (args.Length < 3)
        {
            Console.Error.WriteLine("usage: Patcher <ManagedDir> <ADOFAITrainerMac.dll> <0Harmony.dll>");
            return 2;
        }
        ...
        string backup = asmPath + ".adofaitrainer-backup";
        ...
        File.Copy(trainerSrc, Path.Combine(managed, "ADOFAITrainerMac.dll"), true);
        Console.WriteLine("[copy] 0Harmony.dll + ADOFAITrainerMac.dll -> Managed/");
        ...
        Console.WriteLine($"[patch] inserted call to ADOFAITrainerMac.Loader.Init() at start of {InjectType}.{InjectMethod}");
